...
channel.queue_declare(queue=queueSettings['UnSubscribeQueueName'], durable=True)

print ' [*] Waiting for messages. To exit press CTRL+C'
#Logger(self,moduleName,type,Msg):
LOG.info("UnSubscribe Worker [*] Waiting for messages. To exit press CTRL+C")
...
def callback(ch, method, properties, body):
    LOG.info("UnSubscribe Worker -- Received Request")
    time.sleep(5)
    ...
    msg = objStack.UnSubscribeQueue(EventId, CustomerId, CustomerName, UserName, email, planname, planDetails, OSType)
    LOG.info("UnSubscribe Worker %s"% msg)

    LOG.info("UnSubscribe Worker -- UnSubscribe Status (True/False) %s" % msg['Status'])
    ...
    if properties.headers["retry_count"] > count:
        LOG.info("UnSubscribe Worker -- Saving in DB")
        ch.basic_ack(delivery_tag=method.delivery_tag)
    else:
        try:
            if msg['Status'] == False or msg['Status'] == "Error":
                raise Exception("UnSubscribe can't be done due to some reasons.Re-Queuing the Message again")
            else:
                ch.basic_ack(delivery_tag=method.delivery_tag)
                LOG.info("UnSubscribe Worker -- Successfully Operated and removed from Queue")
                ...
                Notification_Queue(str(UserQueueDetails))
                # End of Throwing Message to Project Queue
        except:
            LOG.info("UnSubscribe Worker -- Just Reached Exception Area")
            LOG.info("UnSubscribe Worker -- Before setting header, Count was %s" % properties.headers["retry_count"])
            ...
            channel.basic_publish(
                exchange='',
                routing_key=queueSettings['UnSubscribeQueueName'],
            ...
            LOG.info("UnSubscribe Worker -- Queue Acknowledged and removed")
            LOG.info("UnSubscribe Worker -- [++++++Done+++++]")

channel.basic_qos(prefetch_count=1)
channel.basic_consume(callback, queue=queueSettings['UnSubscribeQueueName'])
